								...
								const int &maximize, const DESIGN_INFO &D_INFO, const PSO_OPTIONS &PSO_OPTS, const PSO_DYN &PSO_DYN)
{
	...
	//
	MIX_COL_VAL = MIX_OPERATOR(0, MIX_COL, DESIGN, DESIGN_VAL, MAT_TAR, maximize, D_INFO, PSO_OPTS, PSO_DYN);
	if (D_INFO.balance != 1) {
		MIX_ROW_VAL = MIX_OPERATOR(1, MIX_ROW, DESIGN, DESIGN_VAL, MAT_TAR, maximize, D_INFO, PSO_OPTS, PSO_DYN);
	} else {
